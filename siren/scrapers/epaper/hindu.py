...
BASE_URL = URL("https://epaper.thehindu.com")

PUBLICATIONS = {
    "th_bangalore": "Bangalore",
    "th_chennai": "Chennai",
    "th_coimbatore": "Coimbatore",
    "th_delhi": "Delhi",
    "th_erode": "Erode",
    "th_hyderabad": "Hyderabad",
    "th_international": "International",
    "th_kochi": "Kochi",
    "th_kolkata": "Kolkata",
    "th_kozhikode": "Kozhikode",
    "th_madurai": "Madurai",
    "th_mangalore": "Mangalore",
    "th_mumbai": "Mumbai",
    "th_thiruvananthapuram": "Thiruvananthapuram",
    "th_visakhapatnam": "Visakhapatnam",
    "th_tiruchirapalli": "Tiruchirapalli",
    "th_vijayawada": "Vijayawada",
}
...
class TheHinduArticle(ResultModel):
    ...
    def url(self) -> str:
        quoted_url = quote(
            f"{BASE_URL}/ccidist-ws/{self.organization_id}/{self.publication_id}/issues/{self.issue_id}/OPS/{self.articlefilename}",
            safe="",
        )
...
class TheHinduScraper(BaseScraper[TheHinduArticle]):
    ...
    async def search(
        self, keyword: str, publication_id: str, rows: int, start: int = 0
    ) -> TheHinduSearchResult | None:
        start_dt = self.start.isoformat()
        end_dt = self.end.isoformat()
        url = (
            str(BASE_URL)
            + f"/solr/articles/select?fq=os:web&fq=(CONTENT:{keyword} OR articleheadline:{keyword})&fq=organization_id:th&fq=publication_id:{publication_id}&q=issue_date:[{start_dt}Z TO {end_dt}Z]&rows={rows}&start={start}"
        )
    ...
    async def scrape(self) -> list[TheHinduArticle]:
            ...
            for pid in PUBLICATIONS:
                ...
                for i in range(1, total // self.CHUNK_SIZE):
                    task = asyncio.create_task(
                        self.search(kw, pid, self.CHUNK_SIZE, start=i * self.CHUNK_SIZE)
                    )
                    tasks.append(task)
        ...
        for item in results:
            if item.keyword in item.text.lower() and not any(
                kw in item.text.lower() for kw in self.ignore_keywords
            ):
